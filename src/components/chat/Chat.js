...
const Message = (props) => {
    ...
    const textAndEmoties = wordArray.map((word, index) => {    
        ...
        if(emojiObject.name === word) {            
            return <React.Fragment key={index}><img alt={word} src={emojiObject.src} /> </React.Fragment>  
        }
    ...
    return (
        <div className="chat">
            ...
            <div className="users">
                ...
                {(users)?users.map((user, index) => <User key={index} user={user} />):null}
            </div>
            <div>
                <div className="inputBox">
                    <form onSubmit={handleSubmit} >
                        <input name="text" className="input" autoComplete="off" value={form.text} onChange={updateForm} />
                    </form>
                </div>    
