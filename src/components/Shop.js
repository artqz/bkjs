    ...
    render() {        
        ...
        return (
            ...
            <div className="shopSlot" onClick={() => this.selectItem(item)}>
                <div className={(targetItem === item.id)?'itemFrame selected':'itemFrame'}></div>
...
class Shop extends Component {
    ...
    buyItem (itemId, userId) {
        axios.post(`http://127.0.0.1:8000/api/items/`, { item_id: itemId, user_id: userId }).then(res => {
            this.props.buyItem();
        });
    }
    leftTo6(num) {
        ...
        return (
            <div className="shop">       
                ...
                <div className="shopList">
                    {shopList.map((item, index) => <Item selectItem={this.selectItem.bind(this)} key={index} item={item} targetItem={selectItem.id} />)}
                </div>
                {(showItemInfo)? (
                    <div className="shopItemInfo">
                        <div className="shopItemIcon"><img src={pathItems + selectItem.icon} alt={selectItem.name}/></div>                        
                        <div className="shopItemDesc">
                            <div className="name">{selectItem.name}</div>
                            <div className="stat">physical attack: <span className="value">{selectItem.p_atk}</span></div>
                            <div className="price">price: <span className="value">{selectItem.price}</span></div>     
                        </div>                
                        <div className="shopItemBuy" onClick={() => this.buyItem(selectItem.id, playerInfo.id)}>Buy</div>
                    </div>
                ) : null}       
            </div>
        );
